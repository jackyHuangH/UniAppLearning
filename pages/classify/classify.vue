<template name="classify">
	<view>
		...
		<scroll-view scroll-y="true" class="page" lower-threshold="60px" @scrolltolower="scrollToLoadMore" enable-back-to-top="true">
			...
			<!-- 顶部横向滑动tab -->
			<scroll-view scroll-x="true" class="bg-white fixed nav" :style="[{ top: fixedTop + 'px' }]" scroll-with-animation :scroll-left="scrollLeft">
				<view class="cu-item" v-for="(item, index) in tabs" :key="index" :class="index == tabCur ? 'text-blue cur' : ''" @tap="onTabSelected" :data-index="index">
					{{ item.name }}
				</view>
			</scroll-view>
